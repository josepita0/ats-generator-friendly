            ...
            <div key={field.id} className="retro-border rounded-xl bg-white/60 p-4 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                <div className="flex flex-col gap-1">
                  ...
                  <input type="month" {...register(`education.${index}.startDate` as const, { required: dict.validation.required })} className="retro-input p-2 w-full text-sm" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="font-label-md font-bold text-xs">{dict.fields.endDate}</label>
                  <input type="month" {...register(`education.${index}.endDate` as const)} className="retro-input p-2 w-full text-sm" />
                </div>
              </div>
            </div>
          ))}
