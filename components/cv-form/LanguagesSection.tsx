...
export function LanguagesSection({ dict }: Props) {
  const { register } = useFormContext<CVData>();
  const { fields, append, remove } = useFieldArray({ name: 'languages' });
  const [collapsed, setCollapsed] = useState(false);

  return (
    <div className="retro-card bg-[#f890bd] p-8 text-black relative z-0 translate-x-1">
            ...
            <div key={field.id} className="retro-border rounded-xl bg-white/60 p-4 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  <label className="font-label-md font-bold text-xs">{dict.fields.language}</label>
                  <input {...register(`languages.${index}.language` as const, { required: dict.validation.required })} className="retro-input p-2 w-full text-sm" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="font-label-md font-bold text-xs">{dict.fields.level}</label>
                  <input {...register(`languages.${index}.level` as const, { required: dict.validation.required })} className="retro-input p-2 w-full text-sm" placeholder="Native, Fluent, Advanced" />
                </div>
              </div>
            </div>
          ))}
